...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : MonoBehaviour
    {
        ...
        private void Start()
        {
			...
			if (!vertical) {
				float bxXSize = StartObject.transform.position.x-EndObject.transform.position.x;
				bx.size = new Vector3 (Mathf.Abs(bxXSize), bx.size.y, bx.size.z);
				float centreX = (StartObject.transform.position.x + EndObject.transform.position.x) / 2;
				bx.center = new Vector3 (centreX, bx.center.y, bx.center.z);
			} 
			else {
				float bxZSize = StartObject.transform.position.z-EndObject.transform.position.z;
				bx.size = new Vector3 (bx.size.x, bx.size.y, Mathf.Abs(bxZSize));
				float centreZ = (StartObject.transform.position.z + EndObject.transform.position.z) / 2;
